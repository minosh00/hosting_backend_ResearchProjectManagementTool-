...
const upload=multer({
    ...
    fileFilter: (req, file, cb) => {
      if (file.mimetype == "image/png" || file.mimetype == "image/jpg" || file.mimetype == "image/jpeg" || file.mimetype == "application/pdf" || file.mimetype =="/pptx") {
          cb(null, true);
      } else {
          cb(null, false);
...
router.post("/", upload.array('uploaded_Image', 10), async (req, res) => {
    try {
     ...
     for(let i=0;i<fileInArray.length;i++){
       ...
       else if(fileext=="pdf" ||  fileext=="pptx")
